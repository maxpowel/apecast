...
pub async fn latest(db: std::sync::Arc<Database>) -> Result<Option<String>> {
    info!("Running cryptodaily crawler");
    let response = reqwest::get("https://cryptodaily.co.uk/search?q=apecoin").await?.text().await?;
    let document = Html::parse_document(&response);

    let selector = Selector::parse("a.post-item").unwrap();
